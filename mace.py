...
from app.mace.config import ALPHA, BETA, EM, MAX_ITERATIONS, RESTARTS, THRESHOLD, FILLER
...
class Mace(object):
    ...
    def __init__(
        ...
        self.label2int = {value: label for value, label in zip(range(self.num_labels), self.unique_labels)}
        self.label2int.update({FILLER: -1})

        # set int to label dict
        self.int2label = {value: label for label, value in self.label2int.items()}
        ...
        self.active_annotations = [[i for i in range(len(row)) if row[i] > -1] for row in self.labels]
    ...
    def reset_params(self):
        ...
        self.gold_label_marginals = np.zeros(shape=(self.num_instances, self.num_labels))
        self.label_preference_expected_counts = np.zeros(shape=(self.num_annotators, self.num_labels))
        self.competence_expected_counts = np.zeros((self.num_annotators, 2))

        # initialize parameters
        self.competence = np.random.random((self.num_annotators, 2)) + self.smoothing
        self.competence = self.competence / self.competence.sum(axis=1).reshape(-1, 1)

        self.label_preference = np.random.random(shape=(self.num_annotators, self.num_labels)) + self.smoothing
        self.label_preference = self.label_preference / self.label_preference.sum(axis=1).reshape(-1, 1)
        ...
        self.label_preference_priors = np.ones((self.num_annotators, self.num_labels)) * 10.0
            ...
            if self.labels[d].sum() > -self.num_annotators:
                ...
                for l in range(self.num_labels):
                    ...
                    for a in self.active_annotations[d]:
                        ...
                        spam_value = self.competence[a][1] if l == annotation else 0.0
                        gold_label_marginal *= self.competence[a][0] * self.label_preference[a][annotation] + spam_value

                    if not self.controls or self.controls and self.controls[d] == l:
                ...
                for a in self.active_annotations[d]:
                    ...
                    if self.controls:
                        ...
                        if annotation == self.controls[d]:
                            spam_value = self.competence[a][1] if self.controls[d] == annotation else 0.0
                            strategy_marginal += self.gold_label_marginals[d][l] / (
                                self.competence[a][0] * self.label_preference[a][annotation] + spam_value
                            )

                            strategy_marginal *= self.competence[a][0] * self.label_preference[a][annotation]

                            self.label_preference_expected_counts[a][annotation] += (
                                strategy_marginal / instance_marginal
                            )
                            self.competence_expected_counts[a][0] += strategy_marginal / instance_marginal
                            self.competence_expected_counts[a][1] += (
                                self.gold_label_marginals[d][annotation]
                                * self.competence[a][1]
                                / (self.competence[a][0] * self.label_preference[a][annotation] + self.competence[a][1])
                            ) / instance_marginal
                    ...
                    # if controls is not defined
                        ...
                        strategy_marginal *= self.competence[a][0] * self.label_preference[a][annotation]
                        self.label_preference_expected_counts[a][annotation] += strategy_marginal / instance_marginal
                        self.competence_expected_counts[a][0] += strategy_marginal / instance_marginal
                        self.competence_expected_counts[a][1] += (
                            self.gold_label_marginals[d][annotation]
                            * self.competence[a][1]
    ...
    def M_step(self):
        ...
        :return:
        """
        self.competence_expected_counts = self.competence_expected_counts + self.smoothing
        self.competence = self.competence_expected_counts / self.competence_expected_counts.sum(axis=1).reshape(-1, 1)

        self.label_preference_expected_counts = self.label_preference_expected_counts + self.smoothing
        self.label_preference = self.label_preference_expected_counts / self.label_preference_expected_counts.sum(
            axis=1
        ).reshape(-1, 1)
    ...
    def variational_M_step(self):
        ...
        """
        self.competence_expected_counts = self.competence_expected_counts + self.competence_priors
        self.competence = np.exp(ssp.digamma(self.competence_expected_counts)) / np.exp(
            ssp.digamma(self.competence_expected_counts.sum(axis=1).reshape(-1, 1))
        )

        self.label_preference_expected_counts = self.label_preference_expected_counts + self.label_preference_priors
        self.label_preference = np.exp(ssp.digamma(self.label_preference_expected_counts)) / np.exp(
            ssp.digamma(self.label_preference_expected_counts.sum(axis=1).reshape(-1, 1))
        )
    ...
    def fit(self):
        ...
        if len(self.controls) > 0:
            print("Running {} EM training with CONTROLS and the following settings:".format(mode), file=sys.stderr)
        else:
            print("Running {} EM training with the following settings:".format(mode), file=sys.stderr)
        ...
        for restart in range(self.restarts):
            ...
            self.reset_params()
            self.E_step()
            print("initial log marginal likelihood = {}".format(self.log_marginal_likelihood), file=sys.stderr)
            ...
            print("final log marginal likelihood = {}".format(self.log_marginal_likelihood), file=sys.stderr)
        ...
        print("\nTraining completed in {} sec".format(time.time() - start), file=sys.stderr)
        print(
            "Best model came from random restart number {} (log marginal likelihood: {})".format(
                best_restart, best_log_marginal_likelihood
            ),
            file=sys.stderr,
        )
    ...
    def decode_distribution(self):
        ...
        for d in range(self.num_instances):
            if self.labels[d].sum() == -self.num_annotators:
                ...
                probs = self.gold_label_marginals[d] / self.gold_label_marginals[d].sum()
                order = np.argsort(probs)[::-1]
                result.append(list(zip([self.int2label[label] for label in order], probs[order])))
    ...
    def get_label_entropies(self):
        ...
        for d in range(self.num_instances):
            if self.labels[d].sum() == -self.num_annotators:
                ...
                probs = self.gold_label_marginals[d] / self.gold_label_marginals[d].sum()
                entropy = np.where(probs > 0.0, -probs * np.log(probs), 0.0).sum()
    ...
    def get_test(self):
        ...
        ), "Gold labels and input file have different number of instances ({} vs {})".format(
            len(gold), self.num_instances
        )
